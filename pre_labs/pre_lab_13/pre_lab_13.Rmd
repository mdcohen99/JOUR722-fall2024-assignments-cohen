...
**Task** Run the following code. Describe the model output. What do you see? **Answer This block of code is looking at a possible correlation between spending per student in 2021 and the growth of the GDP in the same year. What we see is that there is not any statistical significance to any correlation because we see a p value of 0.6795 — a very large value. The result also gives us a t value, a confidence interval and the correlation value, a small, weak negative correlation that we have, as previously stated, shown to not be significant.**
