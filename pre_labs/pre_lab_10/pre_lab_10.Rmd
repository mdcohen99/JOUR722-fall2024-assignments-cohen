...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer A list of zip codes in Maryland an information about its geometric shape**
...
**Task** Run the following code. Describe the output in the space below. **Answer it's a whole nice pretty map of Maryland zip codes!**
...
**Task** Run the following code. Describe the output in the space below. **Answer Maryland counties with GEOID, coordinates and geometric shape info for all counties nationwide**
...
**Task** Run the following code. Describe the output in the space below. **Answer filtered down to just maryland counties from the same previous list**
...
**Task** Run the following code. Describe the output in the space below. **Answer a big blob of maryland counties but not totally accurate cause it paints over the Chesapeake Bay**
...
**Task** Run the following code. Describe the output in the space below. **Answer all the zip codes can now be found inside the counties and in doing so, makes the Bay a little clearer**
...
**Task** Run the following code. Describe the output in the space below. **Answer list of foreclosures by month in certain zip codes in Maryland**
...
**Task** Run the following code. Describe the output in the space below. **Answer This takes the previous Maryland map that we had and adds in a color scale based on foreclosures in certain zip codes showing some zip codes southeast of Washington D.C. having among the most foreclosures**
...
**Task** Run the following code. Describe the output in the space below. **Answer We have a data set now filtered with the total population variable which will have 63 rows and 27 columns**
...
**Task** Run the following code. Describe the output in the space below. **Answer This is similar to previous Census API code that we've done, but now adds a code for geography that we'll need for the maps.**
...
**Task** Run the following code. Describe the output in the space below. **Answer we now have a much simpler table that we can join together cause the column names have been fixed**
...
**Task** Run the following code. Describe the output in the space below. **Answer We have seen now that Calvert County has the highest rate of foreclosures of any county in Maryland.**
...
**Task** Run the following code. Describe the output in the space below. **Answer Now we have a color map with more contrast that makes it easier to differentiate the counties**
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer This creates even further contrast, and it also shows that nearly all the counties with high foreclosure rates are in southern and eastern Maryland**
