...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer: observational data where the id is the name of the course and seats is the number of seats in the class. Those are the "observations"**
...
**Answer: The code takes the overall data set of 60,000+ observations, then filters it by the "term" and summarizes it by showing us the number of classes that are in each term.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: Now it has been organized from the term with the most classes at the top to the term with the least classes at the end.**
...
**Answer It is now filtered more specifically. We are now seeing how many classes each department is offering in each term instead of just the total number of classes per term.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer We are now seeing the total number of seats available per term. We know there are 4,975 classes in the August 2022 term, and now we know additionally that there are a total of 230,796 seats available if you add up the total number of seats in all 4,975 classes.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer: We now have the seats broken down another way. Adding onto the total seats, we know have the mean number of seats in each class in each term. We also have the median number of seats available in each class in each term.**
...
**Answer: We now see which class has the most seats in each term and which has the least. We also could organize it to see which term has the class with the largest number of seats.**
...
1.  It provides the median number of seats per class, but that doesn't mean every seat is filled. Based on that, what is the average number of students actually enrolled in a class at UMD. The median may be the better way to approach this because of Maryland being a giant school and a lot of large lecture classes. How well is the school able to create small, focused classes, at a flagship institution?

2.  Following up on that, what is the professor to student ratio? Are professors being overwhelmed by the number of students?

3.  How does the number of classes and seats available compare with enrollement data? Is UMD providing enough class opportunities for a growing student population? Is it proportional?
