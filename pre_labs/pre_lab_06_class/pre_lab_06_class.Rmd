...
**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer: 24 rows are in the new data set**
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer: various population data about race and age**
...
tax_data <- read_csv('data/assessment.csv') |> clean_names()
```

### Task 10: Now it's your turn: join this dataframe to other zip code data where the zip code column is text, not numbers.

```{r}
maryland_zcta_assessmenet <- maryland_zcta |> left_join(tax_data, join_by(ZCTA5N == zipcode))
```
