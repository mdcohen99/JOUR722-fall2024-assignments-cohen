...
combined_md_calls <- bind_rows(list(allegany911, carroll911, cecil911))
...
bal911 <- read_csv('data/baltimore_911.csv') |> clean_names() |> mutate(zip_code = as.character(zip_code))
zcta <- read_csv('data/maryland_zcta.csv') |> clean_names() |> mutate(zcta5n = as.character(zcta5n))

bal_calls <- bal911 |> left_join(zcta, join_by(zip_code == zcta5n))
```
...
A1. Carroll had the most, then Cecil, then Allegany
...
A2. The most calls came from 314 Grove Neck Rd in Cecil County. Turns out, that is a recovery center! I didn't notice any obvious outliers in the data when i sorted it by county and by month. In all three counties, the number of calls per month seems to follow a bit of a bell curve, starting on the low end, then peaking in the summer before declining back again in the winter.
...
allegany_by_month <- combined_md_calls |>
  filter(county == "Allegany") |>
  mutate(month = month(date)) |>
  group_by(month) |>
  summarize(total = n()) 

carroll_by_month <- combined_md_calls |>
  filter(county == "Carroll") |>
  mutate(month = month(date)) |>
  group_by(month) |>
  summarize(total = n()) 

cecil_by_month <- combined_md_calls |>
  filter(county == "Cecil") |>
  mutate(month = month(date)) |>
  group_by(month) |>
  summarize(total = n()) 

month_data <- bind_rows(list(allegany_by_month, carroll_by_month, cecil_by_month))
...
**A3. 21217 had the highest number of calls per capita out of all the zip codes. There are seven total zip codes that meet the criteria we filtered for in terms of age.**

**Every single one of the seven zip codes is, relatively, downtown Baltimore. 21217, 21216, 21223, 21201 and 21202 are either the heart of downtown, or some of the neighborhoods east of the harbor that are more stereotypicly associated with crime in the city.**
...
under_18 |>
  group_by(zip_code) |>
  summarize(people = (sum(population)/n())) 

per_capita <- 
  under_18 |>
  group_by(zip_code) |>
  summarize(per_capita = (((n())/(sum(population)/n()))*1000))
...
**A4: Going further from the previous question, now I want to find the locations in zip codes with high Black populations — given Baltimore is a majority Black city — had the most overdose calls. Recent census data showed Baltimore city is about 60% Black, so I'm going to use that as the benchmark for any zip code with a Black population of 60% or higher, since that is as or more representative as the city itself.**

**I found that Pennsylvannia Av and W North Av had 35 calls, the most of any one location in a zip code with an above average Black population.**
