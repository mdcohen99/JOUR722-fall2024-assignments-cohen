...
A1: THERE ARE 66 RECORDS OF VARIOUS SPELLINGS OF PG COUNTY PUBLIC SCHOOLS.

```{r}
pg_schools <- payments |>
  filter(
    str_detect(vendor_name, "PRINCE GEORGES COUNTY PUBLIC SCHOOL") |
    str_detect(vendor_name, "PRINCE GEROGES COUNTY PUBLIC SCHOOLS") |
...
A2: 2021 WAS THE LARGEST YEAR OF FUNDING, TOTALING \$1,470,133,533. THE FUNDING LARGELY INCREASED EVERY YEAR IN ORDER, WITH 2015 AS AN OUTLIER.
...
A3: There are 1,258 examples of payments made by "Capital Projects- University of Maryland". We could use a case_when() function to to organize some of the names of vendors that are close by creating a new column to combine all close spellings into one column. That could be tedious, but may be the best way of figuring it out. A few exmaples could be "A & I INC" vs "A&I INC" vs "A&I INC." or a case like "BECKMAN COULTER INC" which lists several rows with the zip code 60055 and one other row with the zip code 92834. There is also a listing of "BGE and BGE HOME".

```{r}
umd_projects <- payments |>
  filter(
...
A4: The largest fiscal year for stem cell research funding was 2009. There was not been much of a pattern in terms of how funding has changed over time, according to the data.
...
stem_cell_data <- cleaned_md_grants_loans |>
  mutate (stem_cell = case_when(
    str_detect(description, "Stem Cell") ~ "stem_cell",
    str_detect(description, "STEM CELL") ~ "stem_cell",
    !str_detect(description, "Stem Cell") ~ "NA",
    !str_detect(description, "STEM CELL") ~ "NA"
  )
  )

stem_cell_data |>
  filter(stem_cell == "stem_cell") |>
  group_by(fiscal_year) |>
  summarize(total=sum(amount)) |>
  arrange(desc(total))
