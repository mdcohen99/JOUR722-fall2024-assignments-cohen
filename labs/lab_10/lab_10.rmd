...
md_statewide_geo <- md_counties |> inner_join(md_statewide, by=c('geoid'))
```

```{r}
#Create the map
ggplot() +
  geom_sf(data=md_statewide_geo, aes(fill=pct_diff)) +
  theme_minimal() +
...
outages <- read_csv('data/county_outages_march2023.csv') |>
  clean_names() |>
  mutate(per_cust = outage_customers/total_customers)
```

```{r}
#Merge geometries with outage data
joined_outages <- md_statewide_geo |> inner_join(outages, join_by('name' == 'county'))
```

```{r}
#Map it out
ggplot() +
  geom_sf(data=joined_outages, aes(fill=per_cust)) +
  theme_minimal() +
...
joined_ev <- md_statewide_geo |> inner_join(ev, join_by('name' == 'county'))
```

```{r}
#Map out the change
ggplot() +
  geom_sf(data=joined_ev, aes(fill=pct_change)) +
  theme_minimal() 
```

```{r}
#Grab population data from Census
dhc_df <- get_acs(geography = "county",
              variables = c(population = "B01001_001"),
              year = 2021,
              state = "MD") |> 
              clean_names()

all_joined <- joined_ev |> inner_join(dhc_df, join_by('geoid')) 
```

```{r}
#Calculate per capita rate for March 2023
all_joined <- all_joined |>
  mutate(per_capita = (pct_change/estimate)*1000)
```

```{r}
#Map out the per capita rate
ggplot() +
  geom_sf(data=all_joined, aes(fill=per_capita)) +
  theme_minimal() 
```
